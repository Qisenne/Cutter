...
EPSILON_DECAY = 1000
EPISODES = 2000
TOTAL_EXPLORATIONS = 300
N_TRAIN_GRAPH = 3000
NODE_NUM_MIN = 100
NODE_NUM_MAX = 150
PROTOTYPE_TOP_K = 10
UPDATE_PROTOTYPE_FREQ = 10
AFFINE_FREEZE_WARMUP_EP = 200
AFFINE_MSE_THRESHOLD = 1e-3
AFFINE_PATIENCE = 20
...
with tqdm(total=EPISODES, desc="Training Progress", dynamic_ncols=True) as pbar_train:
    for episode in range(EPISODES):
        ...
        # === Freeze affine parameters once converged (paper: "frozen after convergence") ===
        if (not affine_ina_frozen) and (episode >= AFFINE_FREEZE_WARMUP_EP):
            if mse_loss_ina < AFFINE_MSE_THRESHOLD:
                affine_ina_good += 1
            else:
                affine_ina_good = 0
            if affine_ina_good >= AFFINE_PATIENCE:
                affine_ina.freeze()
        ...
        if (not affine_rna_frozen) and (episode >= AFFINE_FREEZE_WARMUP_EP):
            if mse_loss_rna < AFFINE_MSE_THRESHOLD:
                ...
                affine_rna_good = 0
            if affine_rna_good >= AFFINE_PATIENCE:
                affine_rna.freeze()
