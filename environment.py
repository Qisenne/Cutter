...
# === Generic image compression trajectory class ===
class GraphCompressionEpisode:
    def __init__(self, graph, tensor_type, device, stop_ratio):
        ...
        assert all(isinstance(n, int) for n in self.node_list), f"[FATAL] there are illegal nodes in the image: {self.node_list}"
    ...
    def step(self, action, reward):
        ...
        if node_id not in self.compress_graph:
            raise RuntimeError(f"[FATAL] node_id {node_id} not in compress_graph 中")
        ...
        # Ensure that `node_list` and `node_list_map` remain consistent at all times
        self.node_list = list(self.compress_graph.nodes())
        self.node_list_map = {nid: idx for idx, nid in enumerate(self.node_list)}
...
class RNAEpisode(GraphCompressionEpisode):
    ...
    def true_return(self):
        ...
        # === Current connectivity is declining ===
        if self.compress_graph.number_of_nodes() == 0:
            conn_drop = 1.0  # diagram is empty; connectivity is completely lost.
        else:
        ...
        # === important node accidentally deleted ===
        ...
        # === Hyperparameter threshold adjustment ===
        conn_thresh = 0.1
        embed_thresh = 0.5
        delete_thresh = 0

        # === Penalty term calculation (linear mapping) ===
